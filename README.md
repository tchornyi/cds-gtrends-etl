...
Each successful run stores one immutable snapshot in `current_trends`: up to 25
rows (rank 1–25), all sharing a single `snapshot_id` / `snapshot_at`.
...
Each run is an append-only snapshot — rows are never updated or deleted. Trend
history is reconstructed by querying across snapshots.
...
| Column              | Type          | Notes                                                        |
| ------------------- | ------------- | ------------------------------------------------------------ |
| `snapshot_id`       | `UUID`        | Shared by all rows of one run                                |
| `snapshot_at`       | `TIMESTAMPTZ` | Shared by all rows of one run                                |
...
Primary key is `(snapshot_id, rank)`. Schema changes go through migrations —
